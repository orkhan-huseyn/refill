...
func NewRedisCounter(redisCfg config.RedisConfig) RedisCounter {
...
func (c RedisCounter) Take(ctx context.Context, key string, amount int, limit float64, rate float64) (RateLimitResult, error) {
	var res RateLimitResult

	keys := []string{key}
	now := float64(time.Now().UnixNano() / 1e9)

	val, err := c.client.Eval(ctx, luaScript, keys, limit, rate, now, amount).StringSlice()
	if err != nil {
		return res, err
	}
